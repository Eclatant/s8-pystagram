"""웹서비스개발캠프 8기 4주 2회차 과제 tests.py (pystagram용)
pystagram 프로젝트의 posts 앱 디렉터리에 넣고 테스트를 수행하세요.
...
class PostTest(TestCase):
    def setUp(self):
        ...
        self.urls = namedtuple('URL', (
            ...
            # URL패턴의 그룹은 pk.
            lambda pk: reverse('photos:delete', kwargs={'pk': pk}),

            # 개별 게시물 보는 URL name 은 'view_post'이며,
            # URL패턴의 그룹은 pk.
            lambda pk: reverse('photos:view', kwargs={'pk': pk}),

            # 게시물 목록 URL name 은 'list_posts'
            lambda : reverse('photos:list'),
